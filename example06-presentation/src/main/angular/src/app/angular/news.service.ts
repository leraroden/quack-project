...
export class NewsService extends BaseNewsService {
  ...
  getAll(): Observable<News[]> {
    return this.http.get(`${env.apiUrl}/news`, {headers: this.defaultHeaders}).pipe(
      map((body: any[]) => body.map(n => News.fromObject(n)))
    );
  }

  getNewest(): Observable<News> {
    return this.http.get(`${env.apiUrl}/news/newest`, {headers: this.defaultHeaders}).pipe(
